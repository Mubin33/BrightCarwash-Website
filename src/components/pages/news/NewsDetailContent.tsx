...
export function NewsDetailContent({ article, loading = false }: Props) {
  ...
  return (
    <section
      className={`flex py-10 px-4 sm:px-6 md:px-10 lg:px-6 xl:px-40 2xl:px-75 flex-col justify-center items-center gap-12 self-stretch border ${isDark ? "border-white/20 bg-[#1A1A1A]" : "border-[#DFE1E7] bg-white"}`}
    >
      <div className="flex flex-col items-start gap-8 self-stretch max-w-330">
        {/* Main Featured Image */}
        <div className="h-75 sm:h-100 lg:h-112.5 self-stretch rounded-lg relative overflow-hidden">
          <Image
            src={article.image}
            alt={article.title}
            fill
        ...
        {article.images.length > 0 && (
          <div className="flex flex-col sm:flex-row items-start gap-6 self-stretch">
            {article.images.map((img, i) => (
              <div
                key={i}
                className="h-75 sm:h-112.5 flex-1 rounded-lg relative overflow-hidden"
              >
                <Image
                  src={img}

...
<<<<<<< HEAD
    ...
    return (
        <section
            className={`flex py-10 px-4 sm:px-6 md:px-10 lg:px-6 xl:px-40  flex-col justify-center items-center gap-12 self-stretch border ${isDark ? 'border-white/20 bg-[#1A1A1A]' : 'border-[#DFE1E7] bg-white'}`}
=======
  const { theme } = useTheme();
  const isDark = theme === "dark";
  ...
  return (
    ...
    >
      <div className="flex flex-col items-start gap-8 self-stretch max-w-[1320px]">
        ...
        {/* Title */}
        <h1
          className={`font-bebas text-3xl sm:text-4xl lg:text-5xl font-normal leading-[124%] tracking-[0.96px] capitalize ${
            isDark ? "text-white" : "text-[#1D1F2C]"
          }`}
>>>>>>> d90aebd268f3c8304aa24de03d16a9e5f15c7cdc
        >
          {article.title}
        </h1>
        ...
        {/* First Half */}
        <div
          className={`self-stretch font-inter text-base font-normal leading-[160%] ${

...
export function CheckoutStep({ onBack, onSuccess }: Props) {
    ...
    const handleCheckout = async () => {
        ...
        if (success) {
            ...
            await lock(LOCATION_ID, startAt, selectedServices.map((s) => s.variationId));
        }
    };
            ...
            <div className="flex flex-col lg:flex-row items-start gap-4 self-stretch">
                ...
                <div className="flex flex-col items-start gap-4 flex-1 lg:max-w-[400px] self-stretch">
                    ...
                    <div className="flex gap-2 self-stretch">
                        ...
                        <Button onClick={handleCheckout} disabled={!isFormValid || !lockToken || checkoutLoading} isLoading={checkoutLoading} loadingText="Processing..." className="flex-1 py-[14px] px-5 justify-center items-center gap-2 rounded-xl bg-[#0098E8] text-white font-inter text-sm disabled:opacity-50">Confirm & Pay</Button>
                    </div>
                </div>
            </div>
        </div>

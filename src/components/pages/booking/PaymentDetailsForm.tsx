    ...
    return (
        <div className={`flex p-4 sm:p-6 flex-col items-center gap-6 self-stretch rounded-xl border ${isDark ? 'border-white/20 bg-white/[0.04]' : 'border-[#DFE1E7] bg-white'}`}>
            <div className="flex flex-col items-start gap-6 self-stretch">
                <h3 className={`font-inter text-xl font-bold leading-normal ${isDark ? 'text-white' : 'text-[#1D1F2C]'}`}>Payment Details</h3>
                <div className="flex flex-col items-start gap-4 self-stretch">
                    <div className="flex flex-col items-start gap-2 self-stretch">
                        <label className={labelClass}>Card Number</label>
                        <input type="text" placeholder="1234 5678 9012 3456" className={inputClass} value={values.cardNumber} onChange={(e) => update('cardNumber', formatCardNumber(e.target.value))} maxLength={19} />
                    </div>
                    <div className="flex flex-col sm:flex-row items-start gap-4 self-stretch">
                        <div className="flex flex-col items-start gap-2 flex-1">
                            <label className={labelClass}>MM/YY</label>
                            <input type="text" placeholder="MM/YY" className={inputClass} value={values.expiry} onChange={(e) => update('expiry', formatExpiry(e.target.value))} maxLength={5} />
                        </div>
                        <div className="flex flex-col items-start gap-2 flex-1">
                            <label className={labelClass}>CVV</label>
                            <input type="text" placeholder="123" className={inputClass} value={values.cvv} onChange={(e) => update('cvv', formatCVV(e.target.value))} maxLength={3} />
                        </div>
                    </div>
                </div>
                <label className="flex items-center gap-2 self-stretch cursor-pointer">
                    <input type="checkbox" checked={agreed} onChange={(e) => onAgreeChange(e.target.checked)} className="w-4 h-4 rounded accent-[#B23730]" />
                    <span className={`font-inter text-sm ${isDark ? 'text-white/70' : 'text-[#777980]'}`}>I have read and agreed to the Privacy & Policy of Brightside Car Wash.</span>
                </label>
            </div>
        </div>
    );

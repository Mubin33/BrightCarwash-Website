...
export function TrustedByLogos({ companies, isDark }: Props) {
    return (
        <div className="self-stretch grid grid-cols-2 md:grid-cols-4 gap-5">
            {companies.map((company) => (
                ...
                >
                    <div className="relative overflow-hidden" style={{ width: company.width, height: company.height }}>
                        <Image
                            src={company.logo}
                            alt={company.name}
                            fill

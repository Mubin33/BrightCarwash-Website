...
export function LocationFilter() {
    ...
    const [selected, setSelected] = useState(LOCATIONS.find(l => l.id === selectedLocation)?.name || LOCATIONS[0].name);
                    ...
                    {LOCATIONS.map((loc) => (
                        <button
                            ...
                            className={`w-full py-3 px-4 text-left text-sm font-inter transition-colors ${selected === loc.name
                                    ...
                                    : isDark ? 'text-white hover:bg-white/[0.08]' : 'text-[#1D1F2C] hover:bg-[#F8FAFB]'
                                }`}
                        >
                            {loc.name}
                        </button>
